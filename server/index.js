...
// CORS — allow browser requests from any origin (needed on Railway)
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = [
      ...
      'application/msword',
    ].includes(file.mimetype) || /\.(pdf|txt|docx|doc)$/i.test(file.originalname);
    cb(ok ? null : new Error('Only PDF, TXT, and DOCX files are supported'), ok);
  },
});
...
{"results":[{"id":"doc-id","source":"IN/SCJudgments","title":"Full case name","date":"YYYY-MM-DD","court":"Court name","snippet":"2-3 sentence excerpt","score":0.95}]}
...
Return JSON with no markdown fences: {"text":"full document text"}`,
...
List each act/rule with section numbers and how it applies.

## Compliance Requirements
Step-by-step list of what must be done.

## Key Case Law
Relevant SC/High Court precedents with citations and holdings.

## Regulatory Obligations
SEBI/RBI/TRAI requirements if applicable.

## Risk & Penalty Analysis
Specific penalties, fines, imprisonment terms under each act.

## Recommended Actions
Concrete prioritised next steps with timelines.

Cite section numbers, case citations, penalty amounts exactly.`;
...
Clauses that are illegal, unenforceable, or create serious liability. Cite relevant sections.

## 3. Concerns (MEDIUM RISK)
Ambiguous clauses, unfavourable terms, missing protections.

## 4. Observations (LOW RISK)
Minor improvements and best-practice suggestions.

## 5. Missing Clauses
Important standard clauses absent from this contract.

## 6. Indian Law Compliance
Check against: Contract Act 1872, Specific Relief Act, IT Act/DPDP Act, labour laws, FEMA, Consumer Protection Act, Stamp Act.

## 7. Negotiation Points
Top 5 clauses to negotiate with suggested alternative wording.

## 8. Summary Scorecard
| Category | Score (1-10) | Comment |
Use a table.

Cite section numbers and case law.`;
